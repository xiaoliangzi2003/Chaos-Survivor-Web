(() => {
  ...
  const enemyDefs = {
    ...
    speeder: ["急行者", 34, 128, 12, 4, "#ffd166", 2, 20, "dash"],
  ...
  const s = {
    mode: "menu", time: 0, wave: 1, waveLeft: WAVE_1, spawn: 0, kills: 0, shards: 0,
  ...
  function start() {
    Object.assign(s, { mode: "choosing", time: 0, wave: 1, waveLeft: waveDuration(1), spawn: 0, kills: 0, shards: 0, camX: 0, camY: 0, shake: 0, flash: 0, boss: null, pendingNext: false, pendingWin: false });
  ...
  function spawnEnemy(id, x, y) {
    ...
    const e = { id, name: d[0], x: x ?? player.x + Math.cos(a) * dist, y: y ?? player.y + Math.sin(a) * dist, hp: d[1] * scale, maxHp: d[1] * scale, speed: d[2], damage: d[3], r: d[4], xp: d[4] === 25 ? 16 : d[4], color: d[5], behavior: d[8], t: Math.random() * TAU, cd: .8 + Math.random(), flash: 0, hitCd: 0, flip: 1 };
  ...
  function killEnemy(e) { enemies.splice(enemies.indexOf(e), 1); if (s.boss === e) s.boss = null; s.kills++; gems.push({ x: e.x, y: e.y, value: e.xp || 5, phase: Math.random() * TAU }); burst(e.x, e.y, e.boss ? 40 : 12, e.color, e.boss ? 240 : 140); if (e.behavior === "split") for (let i = 0; i < 2; i++) spawnEnemy("speeder", e.x + rand(-40, 40), e.y + rand(-40, 40)); }
  ...
  function updateGems(dt) { for (let i = gems.length - 1; i >= 0; i--) { const g = gems[i], dx = player.x - g.x, dy = player.y - g.y, d = Math.max(1, Math.hypot(dx, dy)); if (d < player.magnet) { const pull = (1 - d / player.magnet) * 520 + 120; g.x += dx / d * pull * dt; g.y += dy / d * pull * dt; } if (d < player.r + 12) { player.xp += g.value; s.shards += g.value; gems.splice(i, 1); } } }
  ...
  function completeWave() { s.pendingWin = s.wave >= TOTAL_WAVES; s.pendingNext = !s.pendingWin; for (const g of gems) { player.xp += g.value; s.shards += g.value; } for (const e of enemies) { player.xp += e.xp || 5; s.shards += e.xp || 5; } enemies = []; bullets = []; enemyBullets = []; hazards = []; gems = []; s.boss = null; if (!checkLevel()) finishWave(); }
